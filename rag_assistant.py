...
class RAGAssistant:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # Try to load existing chunks if available
            if self.vector_store.load():
                self.chunks = getattr(self.vector_store, 'chunks', [])
                print(f"[DEBUG] Loaded {len(self.chunks)} existing chunks during initialization")
    ...
    def ingest_documents(self, force_reprocess: bool = False) -> bool:
        ...
        try:
            ...
            self.console.print("[blue]ℹ️ Maximum 5 documents will be processed from the docs folder[/blue]")
            ...
            if self.vector_store is not None and not force_reprocess and self.vector_store.load():
                # Sync chunks from vector store to RAGAssistant
                self.chunks = getattr(self.vector_store, 'chunks', [])
                print(f"[DEBUG] Synced {len(self.chunks)} chunks from vector store to RAGAssistant")
                
                # Show source document breakdown for loaded chunks
                if self.chunks:
                    source_counts = {}
                    for chunk in self.chunks:
                        source_counts[chunk.source] = source_counts.get(chunk.source, 0) + 1
                    print(f"[DEBUG] Loaded chunks from {len(source_counts)} documents:")
                    for source, count in source_counts.items():
                        print(f"[DEBUG]   {source}: {count} chunks")
            ...
            # Process documents (limit to 5 maximum)
            self.console.print("📄 Processing documents...")
            if self.pdf_processor is None:
                self.pdf_processor = PDFProcessor(self.config['docs_dir'])
            
            # Get all PDF files and limit to 5
            pdf_files = list(self.config['docs_dir'].glob("*.pdf"))[:5]
            text_files = list(self.config['docs_dir'].glob("*.txt"))[:5]
            
            total_files = len(pdf_files) + len(text_files)
            if total_files > 5:
                self.console.print(f"[yellow]⚠️ Found {total_files} documents, limiting to first 5[/yellow]")
                # Re-limit combined files to 5 total
            ...
            print(f"[DEBUG] Processing {len(pdf_files)} PDF files and {len(text_files)} text files (max 5 total)")
            ...
            for pdf_file in pdf_files:
                try:
                    ...
                    if text:
                        ...
                        logger.info(f"Processed PDF: {pdf_file.name} ({len(text)} characters)")
                    else:
            ...
            for text_file in text_files:
                try:
                    ...
                    logger.info(f"Processed text file: {text_file.name} ({len(content)} characters)")
                except Exception as e:
            ...
            print(f"[DEBUG] Successfully processed {len(self.documents)} documents: {list(self.documents.keys())}")
            ...
            # Ensure chunks are synced between vector store and RAGAssistant
            self.chunks = getattr(self.vector_store, 'chunks', self.chunks)
            print(f"[DEBUG] After adding chunks, RAGAssistant has {len(self.chunks)} chunks")
    ...
    def query(self, question: str) -> Dict[str, Any]:
        ...
        print(f"[DEBUG] Number of chunks loaded: {len(self.chunks)}")
        if not self.chunks:
    ...
    def _display_document_stats(self):
        ...
        table = Table(title=f"Document Statistics (Processing {len(self.documents)} documents)")
        ...
        print(f"[DEBUG] Document stats: {len(self.documents)} documents, {total_chars:,} total characters")
    ...
    def _display_chunk_stats(self):
        ...
        # Calculate statistics manually since we might not have get_chunk_statistics
...
{chr(10).join([f"• {source}: {count} chunks" for source, count in source_counts.items()])}""",
            title="Processing Results",
            border_style="blue"
        )
        
        self.console.print(info_panel)
        print(f"[DEBUG] Chunk breakdown: {total_chunks} total chunks from {len(sources)} documents")
        for source, count in source_counts.items():
            print(f"[DEBUG]   {source}: {count} chunks")
    ...
    def display_system_info(self):
        ...
        info = {
            ...
            "Status": {
                ...
                "Documents Loaded": f"{len(self.documents)}/5 (max)",
                "Document Names": ", ".join(list(self.documents.keys())[:3]) + ("..." if len(self.documents) > 3 else ""),
    ...
    def _display_query_result(self, result: Dict[str, Any]):
            ...
            if 'search_results' in result:
                ...
                for search_result in result['search_results']:
                    ...
                    if source not in sources_info:
                        sources_info[source] = []
                    sources_info[source].append(search_result['similarity'])
                
                metadata_text.append(f"📚 Sources used: {len(sources_info)} documents")
                for source, similarities in sources_info.items():
                    avg_sim = sum(similarities) / len(similarities)
                    metadata_text.append(f"   • {source}: {len(similarities)} chunks (avg similarity: {avg_sim:.3f})")
            else:
        ...
        if 'search_results' in result and result['search_results']:
            self.console.print("\n[bold blue]📋 Source Chunks Used:[/bold blue]")
            search_table = Table()
            ...
            for i, search_result in enumerate(result['search_results'][:3]):  # Show top 3
                search_table.add_row(
                    search_result['source'],
                    f"{search_result['similarity']:.3f}",
                    search_result['content'][:150] + "..." if len(search_result['content']) > 150 else search_result['content']
                )
