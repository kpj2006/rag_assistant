...
class ChromaDBVectorStore:
    ...
    def __init__(self, collection_name: str = "rag_documents", 
        ...
        self.chunks = []  # Add chunks tracking for consistency
    ...
    def add_embeddings(self, embeddings: np.ndarray, chunks: List[TextChunk]):
        ...
        # Store chunks for consistency with FAISS implementation
        self.chunks.extend(chunks)
    ...
    def save(self):
        """ChromaDB auto-saves, no explicit save needed"""
        logger.info("ChromaDB data is automatically persisted")
    
    def load(self) -> bool:
        """Load existing chunks from ChromaDB collection"""
        try:
            # Get all documents from the collection
            results = self.collection.get()
            
            # Reconstruct chunks from ChromaDB data
            self.chunks = []
...
class VectorStore:
    ...
    def __init__(self, 
        ...
        self.persist_directory = persist_directory
        self.vector_db_dir = persist_directory
        print(f"[DEBUG] VectorStore initialized with vector_db_dir: {self.vector_db_dir}")
    ...
    def add_chunks(self, chunks: List[TextChunk]):
        ...
        # Sync chunks to wrapper
        self.chunks = getattr(self.vector_store, "chunks", [])
        print(f"[DEBUG] Added {len(chunks)} chunks, wrapper now has {len(self.chunks)} total chunks")
    ...
    def save(self):
        ...
        if hasattr(self.vector_store, 'save'):
            ...
            chunks_count = len(getattr(self.vector_store, 'chunks', []))
            print(f"[DEBUG] Saved {chunks_count} chunks via underlying store")
        else:
            print("[DEBUG] Vector store type doesn't support explicit save (data persisted automatically)")
    ...
    def load(self) -> bool:
        ...
        if hasattr(self.vector_store, 'load'):
            ...
            chunks_count = len(getattr(self.vector_store, 'chunks', []))
            print(f"[DEBUG] Loaded {chunks_count} chunks via underlying store")
        else:
            print("[DEBUG] Vector store type doesn't support explicit load")
            loaded = True  # ChromaDB doesn't need explicit loading
        
        # Sync chunks to wrapper if needed
        self.chunks = getattr(self.vector_store, "chunks", [])
        print(f"[DEBUG] Wrapper now has {len(self.chunks)} chunks")
        return loaded
